  ...
  return (
    <section id="planos" className="bg-cream-100 py-24 sm:py-32">
      <div className="mx-auto max-w-content px-6 sm:px-8 lg:px-10">
        ...
        <p className="mt-6 text-center font-sans text-xs text-marine-500">
        ...
        <AnimatePresence>
          {selectedPlan && (
            <motion.div
              ...
              className="sticky bottom-4 z-20 mt-12 flex flex-col items-center justify-between gap-4 rounded-2xl border border-marine-900/10 bg-white/95 p-5 shadow-xl shadow-marine-900/10 backdrop-blur sm:flex-row sm:gap-6"
            >
              <div className="font-sans text-sm text-marine-800">
                Plano <strong className="font-semibold">{selectedPlan.name}</strong> selecionado
                {" — "}
                <span className="text-marine-600">{formatCurrency(selectedPlan.price)}/mês</span>
              </div>
              <Link
                href="#personalizar"
                className="w-full rounded-full bg-marine-900 px-6 py-3 text-center font-sans text-sm font-medium tracking-wide text-cream-50 transition-transform duration-300 hover:-translate-y-0.5 sm:w-auto"
              >
                Continuar
              </Link>
            </motion.div>

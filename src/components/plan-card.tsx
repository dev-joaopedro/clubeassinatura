  ...
  return (
    <motion.button
      ...
      className={`relative flex h-full flex-col rounded-2xl border p-8 text-left transition-colors duration-300 ${
        selected
          ? "border-coffee-900 bg-coffee-900 text-cream-50 shadow-xl shadow-coffee-900/20"
          : "border-coffee-900/12 bg-white text-coffee-900 hover:border-coffee-900/30"
      }`}
    >
      {plan.highlighted && (
        <span
          className={`absolute -top-3 left-8 rounded-full px-3 py-1 font-sans text-[10px] font-semibold uppercase tracking-[0.15em] ${
            selected ? "bg-gold-400 text-coffee-950" : "bg-gold-500 text-cream-50"
          }`}
        >
          Mais escolhido
        </span>
      ...
      <p className={`mt-2 font-sans text-sm ${selected ? "text-cream-100/70" : "text-coffee-600"}`}>
        {plan.tagline}
      </p>

      <div className="mt-6 flex items-baseline gap-1">
        <span className="font-serif text-3xl">{formatCurrency(plan.price)}</span>
        <span className={`font-sans text-sm ${selected ? "text-cream-100/60" : "text-coffee-500"}`}>
          /mês
        </span>
      </div>
      ...
      <ul className="mt-8 flex flex-1 flex-col gap-3">
        {plan.features.map((feature) => (
          <li key={feature} className="flex items-start gap-2 font-sans text-sm">
            ...
            <span className={selected ? "text-cream-100/90" : "text-coffee-700"}>{feature}</span>
          </li>
        ))}
      </ul>
      ...
      <span
        className={`mt-8 inline-flex items-center justify-center rounded-full py-3 text-center font-sans text-sm font-medium tracking-wide transition-colors duration-300 ${
          selected ? "bg-cream-50 text-coffee-900" : "bg-coffee-900/5 text-coffee-900"
        }`}
      >
